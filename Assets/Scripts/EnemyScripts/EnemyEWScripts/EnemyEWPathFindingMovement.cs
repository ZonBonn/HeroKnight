...
public class EnemyEWPathFindingMovement : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // IsHole();
        // IsWallInFront();
        if(enemyEWAI.currentEnemyStateAction == EnemyEWAI.EnemyEWStateAction.Chase || enemyEWAI.currentEnemyStateAction == EnemyEWAI.EnemyEWStateAction.Patrol)
        {
            MovementPhysicPlatformerHandler();
        }
        else if(enemyEWAI.currentEnemyStateAction == EnemyEWAI.EnemyEWStateAction.Jump)
        {
            IsGroundedVar = enemyEWSensor.IsGrounded();
            JumpPhysicalPlatformerHandler();
            MaintainHorizontalVelocityInAir();
        }
        else if(enemyEWAI.currentEnemyStateAction == EnemyEWAI.EnemyEWStateAction.Hurt)
        {
    ...
    private void JumpPhysicalPlatformerHandler()
    {
        ...
        if (IsGroundedVar == true && rb2d.linearVelocityY < 0.1f && enemyEWAI.GetIsJumping() == false/*&& hasLeaveGround == false*/) // chưa rời khỏi mặt đất
        {
            ...
            float obstacleHeight = enemyEWSensor.GetObstacleHeight(); // kiểm tra chiều cao của chướng ngại vật trước khi quyết định lực nhảy 
            ...
            enemyEWAI.SetIsJumpingTrueOutside();
            // hasLeaveGround = true;
            return;
            // isPressedSpace = false;
        }
        else if(IsGroundedVar == true && rb2d.linearVelocityY < 0.1f && enemyEWAI.GetIsJumping() == true) // khi tiếp đất sau khi nhảy thì các điều kiện này sẽ là hợp lý nhất
        {
            enemyEWAI.SetIsJumpingFalseOutside();
        }
    }
